# Midjourney API Error Handling

| Error | Likely Cause | Fix |
|---|---|---|
| `401 Unauthorized` | Missing or invalid `EVOLINK_API_KEY` | Create a new key and set `export EVOLINK_API_KEY="..."` |
| `400 Bad Request` | Invalid model, prompt parameter, speed mode, task ID, image number, or mask payload | Check the workflow docs and request fields |
...
| `failed` task status | Provider-side generation failure, moderation filtering, or invalid input | Read `error.code` and `error.message`, then retry with corrected input |
